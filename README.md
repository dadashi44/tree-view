...
- `levelLayout: 'stack'` — каждый раунд укладывается сам по себе и целиком
  центруется относительно самого населённого. В дереве место второго раунда
  выводится из первого, поэтому его отступ отдельно не задать; в свайпере раунды
  показываются по одному, и связь больше не нужна;
