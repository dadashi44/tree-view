...
- `levelLayout: 'stack'` — каждый раунд укладывается сам по себе, сверху вниз.
  В дереве место второго раунда выводится из первого, поэтому его отступ отдельно
  не задать; в свайпере раунды показываются по одному, и связь больше не нужна;
...
Цвета и скорость растворения меняются переменными `--tv-rounds-bg`,
`--tv-rounds-color`, `--tv-rounds-border`, `--tv-rounds-fade`.
