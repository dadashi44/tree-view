...
| `hasChildren` | `false` | показать кнопку, сворачивающую предыдущие матчи |
| `collapsed` | `false` | предыдущие матчи сейчас свёрнуты |
| `collapseHint` / `expandHint` | «Свернуть/Показать предыдущие матчи» | подписи кнопки |

Событие `select` отдаёт команду, по которой кликнули, `toggle` — нажатие на кнопку
сворачивания. Всё, что ей нужно, приходит из слота `#node` у `TreeView`:

```vue
<template #node="{ data: match, hasChildren, collapsed, toggle }">
  <BracketCard :match="match" :has-children="hasChildren" :collapsed="collapsed" @toggle="toggle" />
</template>
```

Свёрнутые матчи уходят из раскладки целиком — вместе со своей веткой и линиями.
Цвета меняются переменными
...
раунда (`scroll-snap`). Пройденные раунды плавно растворяются вместе с линиями,
которые из них выходят, — на экране остаются текущий раунд и то, что дальше.

Сетка при этом остаётся одна — линии между оставшимися матчами на месте.
Меняются три настройки, и они приходят в слот готовыми:

- `levelGap` растягивается до ширины экрана (`levelGapForWidth`);
- `levelLayout: 'stack'` — каждый раунд укладывается сам по себе, начиная с одной
  черты: при свайпе матчи любого раунда оказываются сверху, а не уезжают к середине
  экрана (`levelAlign: 'center'` центрует их вместо этого). В дереве место второго
  раунда выводится из первого, поэтому его отступ отдельно не задать; в свайпере
  раунды показываются по одному, и связь больше не нужна;
- `linkStyle: 'bracket'` рисует линии скобкой: пара сходится в одну вертикаль,
  и уже от её середины идёт линия к следующему матчу. От `'elbow'` отличается
  только там, где родитель стоит не по центру детей — то есть как раз в `'stack'`;
- `elbowOffset` сводит линии пары сразу за карточками: промежуток между раундами
  во весь экран, и колено «ступеньки» посередине уехало бы далеко вправо — вместо
  скобки на пару у каждого матча получался бы свой длинный хвост;
- `siblingGap` и `groupGap` разводят матчи по парам: внутри пары, которая сходится
  в один следующий матч, тесно (`swipeSiblingGap`), а между парами промежуток
  крупнее и зависит от числа матчей в раунде (`roundSiblingGap`, до четырёх крат).
  Без этого весь раунд читается одним списком и непонятно, кто с кем сходится.
...
| `swipeSiblingGap` | `12` | базовый отступ между матчами раунда; растёт с их числом до 4× |
...
В свайпере линия — единственная подсказка, что матч идёт дальше, поэтому она
заметнее: `--tv-rounds-link-color` и `--tv-rounds-link-width`. Остальное меняется
переменными `--tv-rounds-bg`, `--tv-rounds-color`, `--tv-rounds-border`,
`--tv-rounds-fade`.

Без Vue то же самое считают функции из core: `buildBracketRounds`, `levelGapForWidth`
(промежуток, при котором раунд занимает всю ширину), `roundSiblingGap`, `roundAtScroll`,
`roundOfDepth`, `roundScrollLeft` и `hideRoundsBefore`.
...
Токен для этого не нужен: доверие между npm и репозиторием установлено через
OpenID Connect (Trusted Publisher, настраивается на странице каждого пакета в npm →
Settings → Trusted Publisher → GitHub Actions). Раннеру выдаётся короткоживущий
идентификатор, поэтому в секретах репозитория не лежит ничего, что можно украсть.
Побочный бонус: npm помечает такие версии значком provenance — видно, из какого
коммита и какого workflow собран пакет.

Повторный запуск прогона безопасен: `scripts/publish-if-new.mjs` пропускает пакет,
если такая версия уже в реестре, — npm не разрешает публиковать поверх, и без этой
проверки перезапуск красил бы весь workflow.
