...
- `siblingGap` становится функцией: отступ зависит от того, сколько матчей
  в раунде (`roundSiblingGap`). В раунде на восемь матчей они иначе сливаются
  в одну стену, а в полуфинале лишнее место просто гонит вниз по экрану.
  Базовое значение — `swipeSiblingGap`, кратность растёт до четырёх.
